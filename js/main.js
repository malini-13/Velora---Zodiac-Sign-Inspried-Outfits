(function () {
    ...
    return `
      <article class="product-card reveal" id="product-${product.id}">
        <div class="product-media">
          <img src="${product.image}" alt="${product.zodiac} zodiac inspired outfit by Velora" loading="lazy" decoding="async" width="640" height="800" onerror="this.onerror=null;this.src='assets/products/placeholder.svg';">
  ...
  function renderCartDrawer() {
    ...
    list.innerHTML = items.map((item) => `
      <article class="cart-item">
        <img src="${item.image}" alt="${item.zodiac} zodiac inspired outfit by Velora" loading="lazy" width="320" height="400" onerror="this.onerror=null;this.src='assets/products/placeholder.svg';">
        <div>
          <h4>${item.name}</h4>
  ...
  function injectProductSchema() {
    const node = document.querySelector("#velora-product-schema");
    if (!node) return;
    ...
    const graph = window.VeloraProducts.map((product) => ({
      ...
      image: [`https://veloraaaaaa.netlify.app/${product.image}`],
      sku: product.id,
      brand: {
        "@type": "Brand",
        name: "Velora"
      ...
      offers: {
        "@type": "Offer",
        url: `https://veloraaaaaa.netlify.app/shop.html#product-${product.id}`,
        priceCurrency: "INR",
